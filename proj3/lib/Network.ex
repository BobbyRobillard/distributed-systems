      ...
      neighbors ->
        workers = Enum.with_index(neighbors)
          |> Enum.map(fn {neighbors, id} ->
            worker(Proj3.Node, [id, neighbors, algorithm], [id: id])
          end)
        Supervisor.start_link(Proj3.Network, [supervisor(Registry, [:unique, :registry]) | workers], name: :network)
        Agent.start_link(fn -> {nodes, 0, System.monotonic_time()} end, name: :global)
        Proj3.Node.send(0, algorithm.init_message("Hello there!"))
        run()
    end
  end
